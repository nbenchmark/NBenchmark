...
internal static class OutcomeBuilder
{
        ...
        IReadOnlyList<string>? categories = null)
    {
        ...
        return input switch
        {
            RunOutcome.Success s => Build(
                ...
                s.Result.Stats,
                s.Result.SampleCount,
                s.Result.AllocatedBytesMean,
                s.Result.Q1Ns,
                s.Result.Q3Ns,
                s.Result.InterquartileRangeNs,
                s.Result.LowerFenceNs,
                s.Result.UpperFenceNs,
        ...
        IReadOnlyList<string>? categories = null)
    {
        ...
        var opsPerSecond = stats is not null ? ThroughputFromNs(stats.MeanNs) : double.NaN;
        var medianOpsPerSecond = stats is not null ? ThroughputFromNs(stats.MedianNs) : double.NaN;

        var totalOperations = autoTune?.TotalBodyInvocations
                              ?? (long)sampleCount + resolvedWarmup;
        ...
        return new MeasurementOutcome
        {
            ...
            Result = new BenchmarkResult
            {
                ...
                MeanNs = stats?.MeanNs ?? 0,
                MedianNs = stats?.MedianNs ?? 0,
                Percentiles = stats?.Percentiles ?? [],
                Histogram = stats?.Histogram,
                MinNs = stats?.MinNs ?? 0,
                MaxNs = stats?.MaxNs ?? 0,
                StandardDeviationNs = stats?.StandardDeviationNs ?? 0,
                StandardErrorNs = stats?.StandardErrorNs ?? 0,
                MarginOfErrorNs = stats?.MarginOfErrorNs ?? 0,
                ConfidenceLevel = options.ConfidenceLevel,
                CoefficientOfVariation = stats?.CoefficientOfVariation ?? 0,
                Q1Ns = q1,
                Q3Ns = q3,
                InterquartileRangeNs = interquartileRange,
                ...
                MedianAbsoluteDeviationNs = stats?.MedianAbsoluteDeviationNs ?? 0,
                MedianConfidenceIntervalLowerNs = stats?.MedianConfidenceIntervalLowerNs,
                MedianConfidenceIntervalUpperNs = stats?.MedianConfidenceIntervalUpperNs,
                AllocatedBytesMean = allocatedBytesMean,
                AllocatedBytesMedian = allocStats?.P50,
                AllocatedBytesP95 = allocStats?.P95,
                AllocatedBytesMax = allocStats?.Max,
                ...
                OutlierDetectorName = options.ResolveOutlierDetector().Name,
