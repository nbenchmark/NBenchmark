...
public static class BenchmarkResultExtensions
{
    public static BenchmarkResult Print(this BenchmarkResult result, ReportDetail detail = ReportDetail.Simple)
    {
        ...
        if (result.MarginOfError > 0)
        {
            ...
            Console.WriteLine($"  │  CI:     [{BenchmarkFormatter.FormatNs(result.ConfidenceIntervalLower)} … {BenchmarkFormatter.FormatNs(result.ConfidenceIntervalUpper)}] ({result.ConfidenceLevel * 100:0.#}%)");
        }
        ...
        if (detail == ReportDetail.Advanced)
        {
            ...
            Console.WriteLine($"  │  Range:  {BenchmarkFormatter.FormatNs(result.Range)} ({BenchmarkFormatter.FormatNs(result.Min)} → {BenchmarkFormatter.FormatNs(result.Max)})");
            ...
            if (result.LowerFence is not null && result.UpperFence is not null)
                Console.WriteLine($"  │  Fences: [{BenchmarkFormatter.FormatNs(result.LowerFence.Value)} … {BenchmarkFormatter.FormatNs(result.UpperFence.Value)}]");
