...
internal static class BodyResolver
{
    ...
    /// <summary>
    ///     Invokes the engine on a resolved body, selecting the correctly-typed overload.
    ///     <para>
    ///         The generic overloads are reached by reflection once per benchmark, outside the
    ///         measured region. What runs inside the loop is a real <c>Func&lt;T&gt;</c> - a
    ///         monomorphic call the JIT can inline - so this costs nothing per operation.
    ///     </para>
    /// </summary>
        ...
        CancellationToken cancellationToken)
    {
        ...
        var open = isAsync
            ? RunAsyncGenericMethod
            : RunSyncGenericMethod;

        var closed = open.MakeGenericMethod(
            isAsync ? returnType.GetGenericArguments()[0] : returnType);

        var invoked = closed.Invoke(BenchmarkRunner.Instance, [name, body, spec, cancellationToken]);

        return isAsync
            ? await (Task<MeasurementOutcome>)invoked!
            : (MeasurementOutcome)invoked!;
    }

    private static readonly MethodInfo RunSyncGenericMethod = typeof(BenchmarkRunner)
        .GetMethods(BindingFlags.Public | BindingFlags.Instance)
        .Single(m => m is { Name: "Run", IsGenericMethodDefinition: true });

    private static readonly MethodInfo RunAsyncGenericMethod = typeof(BenchmarkRunner)
        .GetMethods(BindingFlags.Public | BindingFlags.Instance)
        .Single(m => m is { Name: "RunAsync", IsGenericMethodDefinition: true });
}
