...
- **[Isolated runs](./isolated-runs.md)** - Every mode measures in a freshly spawned worker process by default because JIT tiering and GC flavor are fixed at process start. Use `WithIsolation(Isolation.Off)` to opt a suite back into the host process.
- **[Categories](./categories.md)** - Tag benchmarks with `[BenchmarkCategory]` to include or exclude groups from a run via CLI flags or the `FilterCategories` API.
...
- **[Parameterized benchmarks: Harness mode](./parameterized-harness.md)** - Run a benchmark body across multiple input values using the `[Arguments]` and `[ArgumentsSource]` attributes.
